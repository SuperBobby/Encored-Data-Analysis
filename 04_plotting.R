...
for(i in 2:length(MARG_return_dts)){ 
  ...
  for(windowingWeek in c(4)) {
    ...
    if(grepl("light", plot_name)){
      ...
      save.plot(paste0("../plots/Exp1_",plot_name, "_1.png"), stats)
      save.plot(paste0("../plots/Exp1_",plot_name, "_2.png"), partial_lightON)
      save.plot(paste0("../plots/Exp1_",plot_name, "_3.png"), lightON_duration)
...
for(lab in 1:4){ 
  ...
  aggDay_rownum_expDate <- set.expDate.rownum(strong_light_aggDay_dt, expDate)

  aggWeek_rownum_expDate <- set.expDate.rownum(strong_light_aggWeek_dt, expDate)
  ...
  for(windowingWeek in c(4)) {
    ...
    p1 = add.colorful.window.line(p1, strong_light_aggDay_dt, 'strong_light_80', windowingWeek, strong_light_color, rownum_expDate)
    ...
    p2 = add.colorful.window.line(p2, strong_light_aggWeek_dt, 'strong_light_80', windowingWeek, strong_light_color, rownum_expDate)
...
for(lab in 1:4){ 
  ...
  rownum_expDate <- set.expDate.rownum(full_lightON_aggWeek_dt, expDate)
  ...
  for(windowingWeek in c(4)) {
    ...
    p = add.colorful.window.line(p, full_lightON_aggWeek_dt, 'full_lightON_10', windowingWeek, full_lightON_color, rownum_expDate)
...
for(i in 2:length(return_rs_dts)){ 
  ...
  for(windowingWeek in c(4)) {
    ...
    RS_duration      = add.event.vline.exp3(RS_duration)
    RS_freq          = add.event.vline.exp3(RS_freq)
