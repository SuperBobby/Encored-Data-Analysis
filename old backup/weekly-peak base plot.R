...
avg.peak.base.plot = function(data, target, label, start_date, weeks = 4, average_only = F, new_window = F, ylims){
        ...
        for (start_index in indexes){
                ...
                avg_weekday = mean(stack(weekday_data)$values, na.rm=T) * unit 
                ...
                avg_weekend = mean(stack(weekend_data)$values, na.rm=T) * unit
        ...
        average_title = paste(label, ": Average Daily Usage (", target, " : ", average$time_stamp[1], " ~ for ", weeks, " weeks)", sep="")
        #         weekday_title = paste(label, ": Weekday Peak & Base (", target, ")", sep="")
        #         weekend_title = paste(label, ": Weekend Peak & Base (", target, ")", sep="")
        #         
        weekday_title = paste(label, ": Weekday ", target, sep="")
        weekend_title = paste(label, ": Weekend ", target, sep="")
        ...
        aveg_plot <- ggplot(average, aes(x = time_stamp)) +
                ...
                scale_y_continuous("Average Daily Usage (KW/h)", limits = avg_y_range) +
        ...
        if(!average_only){
                ...
                weekday_plot <- ggplot(weekday, aes(x = weekday_timestamp)) +
                        ...
                        geom_text(aes(y= avg, label=round(avg,2), vjust=-1)) + 
...
avg.peak.base.plot(marg_defalut_table_hours, target="total", label = "MARG",  from_date, weeks = plot_weeks, ylims=140)
avg.peak.base.plot(hcc_defalut_table_hours, target="total", label = "HCC",  from_date, weeks = plot_weeks)
avg.peak.base.plot(ux_defalut_table_hours, target="total", label = "UX",  from_date, weeks = plot_weeks)

avg.peak.base.plot(marg_defalut_table_hours, target="computer", label = "MARG",  from_date, weeks = plot_weeks, ylims=80)
avg.peak.base.plot(hcc_defalut_table_hours, target="computer", label = "HCC",  from_date, weeks = plot_weeks)
avg.peak.base.plot(ux_defalut_table_hours, target="computer", label = "UX",  from_date, weeks = plot_weeks)

avg.peak.base.plot(marg_defalut_table_hours, target="light", label = "MARG",  from_date, weeks = plot_weeks, ylims=30)
avg.peak.base.plot(hcc_defalut_table_hours, target="light", label = "HCC",  from_date, weeks = plot_weeks)
avg.peak.base.plot(ux_defalut_table_hours, target="light", label = "UX",  from_date, weeks = plot_weeks)

avg.peak.base.plot(marg_defalut_table_hours, target="hvac", label = "MARG",  from_date, weeks = plot_weeks, ylims=50)



avg.peak.base.plot(marg_defalut_table_hours, target="computer", label = "MARG", from_date, weeks = plot_weeks)
avg.peak.base.plot(marg_defalut_table_hours, target="light",    label = "MARG", from_date, weeks = plot_weeks)
avg.peak.base.plot(marg_defalut_table_hours, target="hvac",     label = "MARG", from_date, weeks = plot_weeks)
