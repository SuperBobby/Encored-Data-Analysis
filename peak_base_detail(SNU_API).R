## by J.Y.Han 2015.10.30.
## working with SNU API
...
peak.base.detail.analysis(marg_15min, "2015-09-01", "MARG", weeks = 2)
...
peak.base.detail.analysis = function(data, start_date, target, weeks){
        ...
        ## Set the start point(day) indexes
        indexes = seq(from = start, by = unit*7, length.out = weeks)
        
        ## Empty List to collect day_peck & day_base data.frame
        ...
        for (start_index in indexes){
                ...
                for (i in 1:7){ 
                        ...
                        day_peak = quantile(a_day_data[,"sum"], 0.95, na.rm = T)
                        day_base = quantile(a_day_data[,"sum"], 0.05, na.rm = T)
                        
                        # preventing from choice of NA for peak || base
                        total_vector = a_day_data[["sum"]]
                        ...
                        #print(paste("day_peak:", a_day_data[day_peak_index,1], day_peak, "(", a_day_data[day_peak_index, "sum"], ")"))
                        #print(paste("day_base:", a_day_data[day_base_index,1], day_base, "(", a_day_data[day_base_index, "sum"], ")"))
                        #cat("\n")
                        ...
                        peak_row = c(type       = "day_peak",
                                     ...
                                     light      = a_day_data[day_peak_index, "light"], 
                                     computer   = a_day_data[day_peak_index, "computer"], 
#                                      hvac       = a_day_data[day_peak_index, "hvac"], 
                                     etc        = a_day_data[day_peak_index, "etc"], 
#                                      etc        = a_day_data[day_peak_index, "sum"]-a_day_data[day_peak_index, light]-a_day_data[day_peak_index, "computer"],
                                     total      = a_day_data[day_peak_index, "sum"])
                        ...
                        base_row = c(type       = "day_base",
                                     ...
                                     light      = a_day_data[day_base_index, "light"], 
                                     computer   = a_day_data[day_base_index, "computer"], 
#                                      hvac       = a_day_data[day_peak_index, "hvac"], 
                                     etc        = a_day_data[day_peak_index, "etc"], 
#                                      etc        = a_day_data[day_base_index, "sum"]-a_day_data[day_base_index, light]-a_day_data[day_base_index, "computer"], 
                                     total      = a_day_data[day_base_index, "sum"])
